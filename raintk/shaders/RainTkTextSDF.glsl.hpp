...
void main()
{
    vec2 d_tex0_uv_dx = dFdx(v_v2_tex0_uv);
    vec2 d_tex0_uv_dy = dFdy(v_v2_tex0_uv);
...
//void main(void)
//{
//    // To smoothen the edge of the glyphs, we use smoothstep
//    // around a center of 0.5.
...
//    float distf_next_x = texture2D(u_s_tex0, vec2(v_v2_tex0.x+v_f_res, v_v2_tex0.y)).r;
//    float distf_next_y = texture2D(u_s_tex0, vec2(v_v2_tex0.x, v_v2_tex0.y+v_f_res)).r;
//    float max_ddist_dxy = abs(distf_next_x-distf) + abs(distf_next_y-distf);

//    // Multiply by a constant because the default seems
//    // a little too blurry
//    float glyph_fuzz = max_ddist_dxy*0.6;
